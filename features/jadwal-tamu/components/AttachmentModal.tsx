      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col">
        <div className="flex items-center justify-between p-5" style={{ borderBottom: '1px solid #EEEEEE' }}>
          <div>
            ...
            <p className="text-xs text-neutral-gray mt-0.5">{event.attachmentFilename ?? 'Lampiran'}</p>
          </div>
        ...
        <div className="flex-1 overflow-auto p-5 flex items-center justify-center">
          {loading && <Loader2 size={28} className="animate-spin text-green-primary" />}

          {!loading && !url && (
            <p className="text-sm text-neutral-gray">Lampiran tidak ditemukan atau gagal dimuat.</p>
          )}
          ...
          {!loading && url && event.attachmentType === 'link' && (
            <a
              ...
              className="flex items-center gap-2 px-4 py-2.5 bg-green-primary text-white text-sm font-semibold rounded-lg hover:bg-green-secondary transition-colors"
            >
